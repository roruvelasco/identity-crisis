...
## Appendix B: Potential Pitfalls

1. **TCP head-of-line blocking** — If one client's send buffer fills, the server's broadcast loop blocks on that client, stalling all other clients. Mitigation: set socket send buffer sizes, add write timeouts, or use non-blocking I/O (NIO) if problems arise.

2. **Stale input on client disconnect** — If a client disconnects mid-carry, the carried player would be stuck. `GameServer.removeClient()` must also clean up carry state for that player.

3. **JavaFX thread safety** — All scene graph modifications must be on the FX Application thread. The network reader thread MUST use `Platform.runLater()` for any UI callbacks. `LocalGameState` uses volatile fields to avoid this for game data.

4. **`DataOutputStream.writeUTF()` length limit** — Max 65535 bytes of modified UTF-8. Player names and chat messages should be capped well below this.

5. **Module system** — If any class can't access another, check `module-info.java` exports. All packages that are accessed across the module boundary must be exported.
